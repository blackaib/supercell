...
class SCRCPY_client():
    ...
    def start_processing(self, connect_attempts=200):
        ...
        try:
            ...
            raise FileNotFoundError("Couldn't find FFmpeg at path FFMPEG_bin: " + str(FFMPEG_bin))

        self.ffoutthrd = Thread(target=self.stdout_thread, args=())
        self.fferrthrd = Thread(target=self.stderr_thread, args=())
        self.ffinthrd = Thread(target=self.stdin_thread, args=())
